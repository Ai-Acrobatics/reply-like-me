...
export function createReplyEngine(config: ReplyEngineConfig) {
  ...
  async function buildContext(
    ...
    const [contact, profile, recentMessages, similarMessages] = await Promise.all([
      store.getContact(contactId),
      store.getProfile(contactId),
      store.getRecentMessages(contactId, recentMessageLimit),
      findSimilarMessages(incomingMessage, contactId),
    ]);
  ...
  async function findSimilarMessages(
    ...
    try {
      ...
      const results = await pinecone.queryByVector(vector, similarMessageLimit, {
        ...
        is_from_me: true, // Only Julian's messages for style reference
      });

      return (results.matches ?? []).map((match) => ({
        text: (match.metadata?.text as string) ?? "",
        score: match.score ?? 0,
        is_from_me: (match.metadata?.is_from_me as boolean) ?? true,
        contact_name: (match.metadata?.contact_name as string) ?? null,
